...
const { Usuario, Paciente, Medico, SolicitudRegistro, ResultadoValidacion, sequelize } = db;
...
class SolicitudRepository {
  ...
  async rechazarSolicitudYEliminarUsuarioPorResultadoValidacion({ resultadoValidacionId, revisadoPor, motivo_decision }) {
    ...
    try {
      ...
      // Flujo actual: eliminamos solicitud, paciente y usuario

      await ResultadoValidacion.destroy({ where: { solicitud_id: solicitud.id }, transaction: t });
      await SolicitudRegistro.destroy({ where: { id: solicitud.id }, transaction: t });
      await Paciente.destroy({ where: { id: pacienteId }, transaction: t });
      await Usuario.destroy({ where: { id: usuarioId }, transaction: t });

      await t.commit();
      logger.info({ msg: 'Solicitud rechazada y entidades eliminadas por resultado_validacion', resultadoValidacionId, solicitudId: solicitud.id });
      return { solicitudId: solicitud.id, usuario_eliminado: true, paciente_eliminado: true };
    } catch (error) {
      await t.rollback();
      logger.error({ msg: 'Error al rechazar por resultado_validacion', error: error.message });
      throw error;
    }
  }
